...
import { BookOpen, Upload, Bookmark, LayoutDashboard, X } from "lucide-react";
 ...
 * MobileSidebar
 *
 * Sidebar mobile menggunakan Shadcn Sheet (slide-in dari kiri).
 * Konten identik dengan AppSidebar desktop.
 *
 * @example
 * <MobileSidebar
 *   open={mobileOpen}
 ...
 *   documents={documents}
 *   user={user}
 *   onDocumentSelect={(id) => { router.push(`/documents/${id}`); setMobileOpen(false); }}
 * />
 */
export function MobileSidebar({
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="left"
        className="flex w-[260px] flex-col p-0 bg-white border-r border-neutral-100 gap-0"
      >
        {/* ── Header ────────────────────────────────────────── */}
        <SheetHeader className="flex-row items-center justify-between h-14 px-4 border-b border-neutral-100 space-y-0">
          <div className="flex items-center gap-2">
            <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-blue-600">
              <BookOpen className="h-3.5 w-3.5 text-white" />
            </div>
            <SheetTitle className="text-sm font-semibold tracking-tight text-neutral-900">
              Cognify
            </SheetTitle>
          </div>
          <SheetClose asChild>
